class LiabilityTypesController < ApplicationController
  def index
    ...
    @liability_types = @q.result(:distinct => true).includes(:liabilities).page(params[:page]).per(10)
  ...
  def create
    ...
    @liability_type.mortgage = params[:mortgage]
    @liability_type.student_loan = params[:student_loan]
    @liability_type.credit_card = params[:credit_card]
    @liability_type.car_loan = params[:car_loan]
    @liability_type.other_liabilities = params[:other_liabilities]
